...
pub fn execute(args: RestoreArgs, _config: &Config) -> Result<()> {
    ...
    let progress_bar = if args.progress {
        Some(ProgressBar::new_restore_bar(files.len() as u64, total_size))
    } else {
        None
    };

    let restored_files = Arc::new(AtomicU64::new(0));
    let restored_size = Arc::new(AtomicU64::new(0));

    // Restore files
    for file in files {
        if let Some(pb) = &progress_bar {
            let current_files = restored_files.fetch_add(1, Ordering::Relaxed) + 1;
            let _current_size = restored_size.fetch_add(file.size, Ordering::Relaxed) + file.size;
            
            pb.set_message(format!("Restoring: {}", file.path.display()));
            pb.set_position(current_files);
        }

        archive.restore_file_public(&file, &target, &options)?;
    }
